...
REPO_NAME="${REPO_NAME:-jackal-java-server}"
...
# ================= Compose 环境变量 =================
# 每个模块使用独立的镜像名、容器名和 compose 项目名，互不影响。
APP_MODULE="$MODULE"
...
APP_IMAGE="${MODULE}:${IMAGE_VERSION}"
export APP_IMAGE
...
# ================= 构建与启动 =================
echo "构建并启动服务..."
docker compose -p "$COMPOSE_PROJECT_NAME" -f "$COMPOSE_FILE" up -d --build || { echo "Docker Compose 启动失败"; exit 1; }
...
echo "部署完成: $APP_CONTAINER_NAME -> $APP_IMAGE"
